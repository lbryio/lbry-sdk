...
def _get_next_available_file_name(download_directory: str,
                                  file_name: str) -> str:
...
async def get_next_available_file_name(loop: asyncio.AbstractEventLoop,
                                       download_directory: str,
                                       file_name: str) -> str:
    return await loop.run_in_executor(None, _get_next_available_file_name,
...
class ManagedStream:
    ...
    __slots__ = [
        ...
        'content_fee', 'downloader', 'analytics_manager', 'fully_reflected',
        'file_output_task', 'delayed_stop_task', 'streaming_responses',
        'streaming', '_running', 'saving', 'finished_writing',
        'started_writing', 'finished_write_attempt'
    ]
    ...
    def __init__(
        ...
        self.downloader = StreamDownloader(
            self.loop, self.config, self.blob_manager, sd_hash, descriptor)
        ...
        self.streaming_responses: typing.List[
            typing.Tuple[Request, StreamResponse]] = []
    ...
    def written_bytes(self) -> int:
        return 0 if not self.output_file_exists else os.stat(
            self.full_path).st_size
    ...
    async def start(self,
        ...
        try:
            await asyncio.wait_for(
                self.downloader.start(node), timeout, loop=self.loop)
    ...
    async def stream_file(self,
        ...
        try:
            async for blob_info, decrypted in self._aiter_read_stream(
                    skip_blobs, connection_id=self.STREAMING_ID):
                if not wrote:
                    decrypted = decrypted[first_blob_start_offset:]
                if (blob_info.blob_num == len(self.descriptor.blobs) - 2) or (
                        len(decrypted) + wrote >= size):
                    decrypted += (
                        b'\x00' * (size - len(decrypted) - wrote -
                                   (skip_blobs * (MAX_BLOB_SIZE - 1))))
                    log.debug("sending browser final blob (%i/%i)",
                              blob_info.blob_num + 1,
                              len(self.descriptor.blobs) - 1)
                    await response.write_eof(decrypted)
                else:
                    log.debug("sending browser blob (%i/%i)",
                              blob_info.blob_num + 1,
                              len(self.descriptor.blobs) - 1)
            ...
            log.warning(
                "connection was reset after sending browser %i blob bytes",
                wrote)
    ...
    async def _save_file(self, output_path: str):
        ...
        try:
            with open(output_path, 'wb') as file_write_handle:
                async for blob_info, decrypted in self._aiter_read_stream(
                        connection_id=self.SAVING_ID):
                    log.info("write blob %i/%i", blob_info.blob_num + 1,
                             len(self.descriptor.blobs) - 1)
                    await self.loop.run_in_executor(
                        None, self._write_decrypted_blob, file_write_handle,
                        decrypted)
    ...
    def set_claim(self, claim_info: typing.Dict, claim: 'Claim'):
        self.stream_claim_info = StoredStreamClaim(
            ...
            claim_info['height'],
            binascii.hexlify(
                claim.to_bytes()).decode(), claim.signing_channel_id,
            claim_info['address'], claim_info['claim_sequence'],
            claim_info.get('channel_name'))

    async def update_content_claim(
            self, claim_info: typing.Optional[typing.Dict] = None):
        if not claim_info:
    ...
    def _prepare_range_response_headers(
            self, get_range: str
    ) -> typing.Tuple[typing.Dict[str, str], int, int, int]:
        if '=' in get_range:
