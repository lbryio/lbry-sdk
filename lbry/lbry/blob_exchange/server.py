...
class BlobServerProtocol(asyncio.Protocol):
        ...
        while self.transport:
            try:
                await asyncio.wait_for(
                    self.started_transfer.wait(),
                    self.idle_timeout,
                    loop=self.loop)
            except asyncio.TimeoutError:
                log.debug("closing idle connection from %s",
    ...
    def connection_made(self, transport):
        ...
        self.peer_address_and_port = "%s:%i" % self.transport.get_extra_info(
            'peername')
        self.blob_manager.connection_manager.connection_received(
            self.peer_address_and_port)
    ...
    def connection_lost(self, exc: typing.Optional[Exception]) -> None:
        ...
        self.blob_manager.connection_manager.incoming_connection_lost(
            self.peer_address_and_port)
        self.transport = None
    ...
    def send_response(self, responses: typing.List[blob_response_types]):
        ...
        self.blob_manager.connection_manager.sent_data(
            self.peer_address_and_port, len(serialized))
        ...
        if data:
            ...
            try:
                ...
                log.error(
                    ...
                    self.peer_address_and_port, len(self.buf + data), '' if
                    not data else binascii.hexlify(self.buf + data).decode())
                self.close()
                return
...
class BlobServer:
    ...
    def start_server(self,
                     port: int,
                     interface: typing.Optional[str] = '0.0.0.0'):
        if self.server_task is not None:
            raise Exception("already running")

        async def _start_server():
            server = await self.loop.create_server(
                lambda: self.server_protocol_class(
                    self.loop, self.blob_manager, self.lbrycrd_address, self.
                    idle_timeout, self.transfer_timeout), interface, port)
