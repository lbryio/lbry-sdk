...
class BlobExchangeClientProtocol(asyncio.Protocol):
    def __init__(
            ...
            peer_timeout: typing.Optional[float] = 10,
    ...
    async def _download_blob(
        ...
        try:
            ...
            response: BlobResponse = await asyncio.wait_for(
                self._response_fut, self.peer_timeout, loop=self.loop)
            ...
            await asyncio.wait_for(
                self.writer.finished, self.peer_timeout, loop=self.loop)
            log.info(
                "%s at %fMB/s", msg,
    ...
    async def download_blob(
        ...
        try:
            self._blob_bytes_received = 0
            self.blob, self.writer = blob, blob.get_blob_writer(
                self.peer_address, self.peer_port)
            ...
            log.warning("race happened downloading %s from %s:%s", blob_hash,
                        self.peer_address, self.peer_port)
    ...
    def connection_made(self, transport: asyncio.Transport):
        ...
        if self.connection_manager:
            self.connection_manager.connection_made(
                f"{self.peer_address}:{self.peer_port}")
        log.debug("connection made to %s:%i", self.peer_address,
                  self.peer_port)

    def connection_lost(self, reason):
        if self.connection_manager:
            self.connection_manager.outgoing_connection_lost(
                f"{self.peer_address}:{self.peer_port}")
        log.debug("connection lost to %s:%i (reason: %s, %s)",
                  self.peer_address, self.peer_port, str(reason),
                  str(type(reason)))
        self.close()
...
async def request_blob(loop: asyncio.AbstractEventLoop, blob: Optional['AbstractBlob'], address: str,
    ...
    try:
        if not connected_protocol:
            await asyncio.wait_for(
                loop.create_connection(lambda: protocol, address, tcp_port),
