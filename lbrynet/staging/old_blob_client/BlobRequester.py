...
from lbrynet.error import PriceDisagreementError, DownloadCancelledError, InsufficientFundsError
# from lbrynet.staging.old_bx_client import ClientRequest, ClientBlobRequest
...
class RequestHelper:
    ...
    def _request_failed(self, reason, request_type):
        if reason.check(DownloadCancelledError, RequestCanceledError, ConnectionAborted,
...
def _handle_download_error(err, peer, blob_to_download):
    if not err.check(DownloadCancelledError, PriceDisagreementError, RequestCanceledError,
...
class DownloadRequest(RequestHelper):
    ...
    def _can_pay_peer(self, blob, arg):
        return (
            blob.length != 0 and
            (not isinstance(arg, Failure) or arg.check(DownloadCancelledError))
        )
    ...
    def _download_failed(self, reason):
        if not reason.check(DownloadCancelledError, PriceDisagreementError):
            self.update_local_score(-10.0)
        return reason
