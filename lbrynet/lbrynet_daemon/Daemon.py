...
class Daemon(AuthJSONRPCServer):
    ...
    def _publish_stream(self, name, bid, claim_dict, file_path=None):
        ...
        if not file_path:
            claim_out = yield publisher.update_stream(name, bid, claim_dict)
        else:
            claim_out = yield publisher.publish_stream(name, bid, claim_dict, file_path)
    ...
    def jsonrpc_publish(self, name, bid, metadata=None, file_path=None, fee=None, title=None,
        ...
        if 'fee' in metadata:
            ...
            new_fee_dict={'version':'_0_0_1' ,'currency': currency , 'address':address , 'amount':fee_dict['amount']}
        ...
        claim_dict = {'version':'_0_0_1','claimType':'streamType',
    ...
    @AuthJSONRPCServer.auth_required
    def jsonrpc_channel_new(self, channel_name, amount=1):
        """
        WORK IN PROGRESS
        Make a channel claim, or update if already claimed
        by user.
        Args:
            'channel_name': (str) name of channel, must start with @
            'amount'(optional): (float) amount of LBRY credits to use for
                                channel claim, default is 1.
        Returns:
        ...
        """
        if channel_name[0] != '@':
            raise Exception('channel_name must start with @')
        d = self.session.wallet.channel_claim(channel_name, amount)
        d.addCallback(lambda r: self._render_response(r))
        return d
